  ...
  db.query(`SELECT * FROM topic`, function(error, topics){
    ...
    db.query("SELECT * FROM topic LEFT JOIN author ON topic.author_id=author.id WHERE topic.id=?",
      ...
      var html = template.HTML(topic[0].title, list,
        ...
        <p>${sanitizeHtml(topic[0].description)}</p>
  ...
  db.query(`SELECT * FROM topic`, function(error, topics){
    ...
    db.query(`SELECT * FROM author`, function(error2, authors){
      ...
      var html = template.HTML(create_title, list,
        `
        <h2>${sanitizeHtml(create_title)}<h2>
        <form action="/create_process" method="post">
          <p><input type="text" name="title" placeholder="title"></p>
          <p><textarea name="description" placeholder="description"></textarea></p>
          <p>${sanitizeHtml(template.authorSelect(authors))}</p>
    ...
    db.query("SELECT * FROM topic WHERE id=?",[queryData.id],function(error2, topic){
      ...
      db.query(`SELECT * FROM author`, function(error3, authors){
        ...
        var html = template.HTML(title, list,
          ...
          <p><input type="hidden" name="id" value="${sanitizeHtml(topic[0].id)}"></p>
          <p><input type="text" name="title" value="${sanitizeHtml(topic[0].title)}" placeholder="Title"></p>
          <p><textarea name="description" placeholder="Description" rows="8" cols="80">${sanitizeHtml(topic[0].description)}</textarea></p>
          <p>${sanitizeHtml(template.authorSelect(authors, topic[0].author_id))}</p>
